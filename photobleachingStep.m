...
trace_id = 22; % which trace (column) from .xlsx file you want to analyze
background_id = 6; % which trace (column) from .xlsx file is the background trace
...
s = 0.5:0.5:50; % vector of possible sigma values for optimization
u = meanValue-50:0.5:meanValue+50;
...
temp_x = x;
temp_x((temp_x < (meanValue + 5*opt_s1)) | (temp_x > (meanValue + 235))) = 0;
temp_x(temp_x > 0) = 1;
...
u = meanValue2-150:0.5:meanValue2+100;
...
time_axis = linspace((n_orig - n_new) * 0.04, n_orig * 0.04, n_new); % x - time axis
...
xlim([((n_orig - n_new) * 0.04) ((n_orig * 0.04) + 1)]);
...
plot(time_axis(1375:2375), data_trim(1375:2375), 'LineWidth', 1.5)
xlim([55 95]);
%ylim([55 505]);
set(gca, 'FontSize', 17, 'FontWeight', 'bold', 'YMinorTick', 'off', 'XMinorTick', 'off', 'box', 'on', LineWidth = 1.5);
